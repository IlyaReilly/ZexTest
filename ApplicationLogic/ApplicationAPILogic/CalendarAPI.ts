...
export class CalendarAPI extends BaseAPI {
    ...
    async CreateAppointmentRequest(title : string, user : string, attendees : string, body : string) {
        var response = await this.page.request.post(`${this.soapServiceUrl}${this.createAppointmentRequest}`, {
            data: {
          ...
          var responceBody = JSON.parse((await response.body()).toString());
          var id = responceBody.Body.CreateAppointmentResponse.echo[0].m[0].id;
          return id;
    }
