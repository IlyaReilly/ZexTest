...
test.describe("Files tests", async () => {
  ...
  async function UploadFileAndOpenDetails({ apiManager, pageManager }) {
    ...
    await pageManager.sideMenu.OpenMenuTab(
      pageManager.sideMenu.SideMenuTabs.Files
    );
    await pageManager.sideSecondaryFilesMenu.OpenSecondaryMenuTab(
      pageManager.sideSecondaryFilesMenu.Tabs.Home
    );
  ...
  test("File with JPG extension can be uploaded", async ({ pageManager }) => {
    await pageManager.headerMenu.UploadNewFile(
      "./TestData/Files/testFile2.jpg"
    );
    await pageManager.sideMenu.OpenMenuTab(
      pageManager.sideMenu.SideMenuTabs.Files
    );
    await expect(
      pageManager.filesList.Elements.File.locator(`"${fileNameJpg}"`)
    ).toContainText("testFile2");
  });
  ...
  test("File can be downloaded", async ({ apiManager, pageManager }) => {
    try {
      ...
      await expect(
        pageManager.filesList.Elements.File.locator(`"${unicFileName}"`)
      ).toBeVisible();
      await pageManager.filesList.OpenFileDetails(unicFileName);
      expect(
        fs.existsSync(await pageManager.fileDetails.DownloadFile())
      ).toBeTruthy();
    } catch (e) {
      throw e;
    }
  ...
  test("File can be permanently removed", async ({
    apiManager,
    pageManager,
  }) => {
    test.slow();
  ...
  test("File must be flagged and unflagged", async ({
    apiManager,
    pageManager,
  }) => {
    await UploadFileAndOpenDetails({ apiManager, pageManager });
